...
public partial class Extensions
{
    ...
    /// <summary>
    ///     Locks the current <see cref="Semaphore"/> and takes its state and wraps it in a disposable, for use in using statements.
    /// </summary>
    /// <param name="semaphore">The semaphore</param>
    /// <returns>A disposable Semaphore lock</returns>
    public static IDisposable Take(this Semaphore semaphore)
    {
        semaphore.WaitOne();
    ...
    /// <summary>
    ///     Locks the current <see cref="SemaphoreSlim"/> and takes its state and wraps it in a disposable, for use in using statements.
    /// </summary>
    /// <param name="semaphore">The semaphore</param>
    /// <returns>A disposable Semaphore lock</returns>
    ...
    internal readonly struct ScopedSemaphoreLock : IDisposable
    {
    ...
    public static IEnumerable<T> GetFlags<T>([NotNull] this T input) where T : Enum
        => Enumerable.Cast<T>(Enum.GetValues(Guard.Require(input).GetType())).Where(e => input.HasFlag(e));
